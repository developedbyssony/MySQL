-- 1. 데이터베이스 생성 명령
-- DB 생성 시 한글을 쓸 수 있도록 -> default character set utf8;
-- cont + enter => 실행 // 한줄 주석
create database ict_practice2 default character set utf8;

/* 우측 하단의 schemas 클릭 후, 새로고침 한 다음 ict_practice2 우클릭
-> set as default schema를 클릭하세요. 이러면 현재 명령을 받을 DB로 선택됩니다.//여러줄 주석 */
...
/* 데이터 적재하기
INSERT INTO 테이블명 (컬럼1, 컬럼2...) VALUE(S) (컬럼 1값, 컬럼 2값, ...);
...
INSERT INTO users value ('qwer123','qwer','쿼리문','query@query.com');

-- 여러분이 계정 3개를 더 넣어주시되, 1개 계정은 이메일 없이 넣어주세요.
-- 이메일 없이 넣는 경우는 id, pw, user_name에만 값을 넣겠다고 선언해야 합니다.
INSERT INTO users value ('aaa123','aaaa','쿼리문','query@query.com');
INSERT INTO users value ('bbb126', 'bbbb', '쿼리문','query@query.com');
-- 이메일 생략을 할 때는 테이블명 뒤에 email을 뺀 나머지만 기입해서 넣거나
-- 혹은 전체 컬럼에 값을 넣어주되 email 자리에 null을 넣어줍니다. 
INSERT INTO users (id, pw, user_name) value ('ccc123', 'cccc', '쿼리문');
INSERT INTO users value ('bbb123', 'bbbb', '쿼리문', null);

/* 데이터 조회하기
SELCT 컬럼1, 컬럼2,... FROM 테이블명;
을 적으면 테이블에 적재된 데이터 조회 가능
SELET * FROM 테이블명; 으로 조회시 해당 테이블 전체 컬럼 조회 가능 */
-- 컬럼명 기입으로 users 테이블의 전체 자료를 조회해보세요.
SELECT id, pw, user_name FROM users;
