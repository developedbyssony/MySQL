...
public interface Vehicle {
	...
	int CAR_MAX_GAS = 70; /// :) 최대 연료량 상수로 선언
	
	// 3. 자동차의 상수 목록을 만들어보겠습니다.
	// 상수화 시킬 여지가 있는 것들을 전부 상수화 시켜주시고, Car에도 적용해주세요.
	int CAR_MAX_SPEED = 200; /// :) 최대 속도량 상수로 선언
	int CAR_MIN_SPEED = 0; // 최소 속도량 상수로 선언
	int CAR_FILL = 30; /// :) 주유 1회량 상수로 선언
	int CAR_DECREASE_GAS = 1; // 연료량 1회 깎였을때 값 상수로 선언
	int CAR_ACCELERATE = 10; /// :) 가속 1회 밟았을 때 값을 상수로 선언
	int CAR_REDUCE = 10; /// :) 감속 1회 밟았을 때 값을 상수로 선언
	
	// 4. 자동차에 이어 기차를 만들어보겠습니다.
	// Train 클래스로 만들어주시고 역시 Vehicle을 구현합니다.
	// 최대속도 400, 연료 충전은 700씩, 연료 소모는 30씩, 증가속도는 100, 감속시 50이 감속됩니다.
	int TRAIN_MIN_SPEED = 0;
	int TRAIN_MAX_SPEED = 400;
	int TRAIN_MAX_GAS = 30;
